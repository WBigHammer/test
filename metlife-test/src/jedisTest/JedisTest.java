...
public class JedisTest {
	...
	public static void testString() {
		...
		jedis.set("foo", "foo update");
		// 获取老数据并更改数据
		System.out.println(jedis.getSet("foo", "foo modify"));//输出foo update
		// 截取value的值
		System.out.println(jedis.getrange("foo", 2, 5));//返回foo modify中第3到6的值o mo
	...
	public static void testList() {
		...
		try {
			...
			// 第一个是key，第二个是起始位置，第三个是结束位置，jedis.llen获取长度 -1表示取得所有-2倒数第二个
...
//[Hello how are you?, Fine thanks. I'm having fun with redis., I should look into this NOSQL thing ASAP]
		...
		// 字串
		System.out.println(jedis.lrange("lists", 0, 3));//[LinkedList, ArrayList, vector]
		// 修改列表中单个值
		jedis.lset("lists", 0, "hello list!");
		// 获取列表指定下标的值
		System.out.println(jedis.lindex("lists", 0));
		// 删除列表指定的值 --lrem(String key, long count, String value)
	...
	public static void testSet() {
		...
		try {
			...
			// 判断 4 是否是myset集合的元素
			System.out.println(jedis.sismember("myset", "4"));//sismember(String key, String member)
			// 移除4
			jedis.srem("myset", "4");
			jedis.srem("myset", "3");
			jedis.srem("myset", "2");
	...
	public static void sortedSet() {
		...
		try {
			...
			System.out.println(setValues);
			Double setsort=jedis.zscore("hackers","Linus Torvalds");
	...
	public static void testHash() {
		...
		try {
			...
			jedis.hdel("kid","name"); //删除map中的某个键值
			System.out.println(jedis.hmget("kid", "name")); // 因为删除了，所以返回的是null
		...
		// 清空数据
		System.out.println(jedis.flushDB());
		// 添加数据：将key、filed、value同时添加进redis中，区别于上边try...catch中把一个map放入radis中
		...
		System.out.println(jedis.hincrBy("hashs", "entryKey", 123));
		// 获取所有的keys
...
//			System.out.println(jedis.keys("*shs"));// 返回的库中key like *shs
//			System.out.println(jedis.del("sanmdde"));// 删除key为sanmdde的对象 删除成功返回1
//														// 删除失败（或者不存在）返回 0
//			System.out.println(jedis.ttl("hashs"));// 返回给定key的有效时间，如果是-1则表示永远有效
//			jedis.setex("timekey", 10, "min");// 通过此方法，可以指定key的存活（有效时间） 时间为秒
//			Thread.sleep(1000);// 睡眠1秒后，剩余时间将为<=9
//			System.out.println(jedis.ttl("timekey")); // 输出结果为9
...
////			System.out.print("\t");
////		}
//		System.out.println(jedis.get("age1" + 9999));
//		System.out.println(jedis.keys("*")); // 返回当前库中所有的key
...
//		// 修饰符(modifier)进行排序。
//		sortingParameters.limit(0, 10);// 可用于分页查询
...
//			jedis.rpush("a", i + "");
//		}
//		int pages=i%pageSize==0?i/pageSize:i/pageSize+1;
