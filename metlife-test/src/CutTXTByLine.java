...
public class CutTXTByLine {
	...
	public static void main(String[] args) {
		System.out.println("begin文件分割");
		Long startTime = System.currentTimeMillis();
		String file2Cut="E:/cutByLine/source/"+args[0];//待分割文件
		String devFile="E:/cutByLine/devisionFiles/"+args[0];//分割后文件存放位置
		int columns=Integer.parseInt(args[1]);//列数
		int rows=Integer.parseInt(args[2]);//每一个子文件中存放行数
		CutTXT(file2Cut,devFile,columns,rows);
		Long endTime = System.currentTimeMillis();
		Long time = endTime - startTime;
		System.out.println("总耗时：" + time + "毫秒");
		System.out.println("文件分割end");
	}
